...
def get_args_parser():
    ...
    parser.add_argument('--train_data', default="./exp_settings/SIIM-ACR/train_list.csv")
    parser.add_argument('--test_data', default='./exp_settings/SIIM-ACR/test_list.csv')
...
def main_SIIM(args):
    ...
    testing_dataloader = DataLoader(testing_dataset, args.batch_size, num_workers=args.num_workers)
    ...
    test_epoch = evaluate_without_mask_mse_bbox
...
def evaluate_without_mask_2class(dataloader,model,device):
    ...
    auc = roc_auc_score(confusion_label, out_cat[:, 1])
